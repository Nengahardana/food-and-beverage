...
$sql2 = $koneksi->query("SELECT * from produk ")->fetch_all(MYSQLI_ASSOC);
...
$cek_data8 = mysqli_query($koneksi, "SELECT pemesanan_produk.*,produk.nama_menu,produk.harga
                  FROM pemesanan_produk
                  LEFT JOIN produk ON  pemesanan_produk.id_menu =produk.id_menu where nama_menu='Es Jeruk'");

$cek_data9 = mysqli_query($koneksi, "SELECT pemesanan_produk.*,produk.nama_menu,produk.harga
                  FROM pemesanan_produk
                  LEFT JOIN produk ON  pemesanan_produk.id_menu =produk.id_menu where nama_menu='Jus Alpukat'");

$cek_data10 = mysqli_query($koneksi, "SELECT pemesanan_produk.*,produk.nama_menu,produk.harga
                  FROM pemesanan_produk
                  LEFT JOIN produk ON  pemesanan_produk.id_menu =produk.id_menu where nama_menu='Jus Mangga'");
$cek_data11 = mysqli_query($koneksi, "SELECT pemesanan_produk.*,produk.nama_menu,produk.harga
                  FROM pemesanan_produk
                  LEFT JOIN produk ON  pemesanan_produk.id_menu =produk.id_menu where nama_menu='Teh Obeng'");
$cek_data12 = mysqli_query($koneksi, "SELECT pemesanan_produk.*,produk.nama_menu,produk.harga
                  FROM pemesanan_produk
                  LEFT JOIN produk ON  pemesanan_produk.id_menu =produk.id_menu where nama_menu='Air Mineral'");
...
foreach ($sql2 as $row) {
    ...
    if ($nama_menu == 'Bakso Urat') {
        ...
        $harga = $row["harga"];
        $total_menu = $harga * $jumlah;
    } else if ($nama_menu == 'Es Jeruk') {
        $jumlah = mysqli_num_rows($cek_data8);


        $harga = $row["harga"];
        $total_menu = $harga * $jumlah;
    } else if ($nama_menu == 'Jus Alpukat') {
        $jumlah = mysqli_num_rows($cek_data9);


        $harga = $row["harga"];
        $total_menu = $harga * $jumlah;
    } else if ($nama_menu == 'Jus Mangga') {
        $jumlah = mysqli_num_rows($cek_data10);


        $harga = $row["harga"];
        $total_menu = $harga * $jumlah;
    } else if ($nama_menu == 'Teh Obeng') {
        $jumlah = mysqli_num_rows($cek_data11);


        $harga = $row["harga"];
        $total_menu = $harga * $jumlah;
    } else if ($nama_menu == 'Air Mineral') {
        $jumlah = mysqli_num_rows($cek_data12);


        $harga = $row["harga"];
        $total_menu = $harga * $jumlah;
    }
    $total_harga += $total_menu;
...
$pdf->Cell(80);
$pdf->Cell(30, 10, 'LAPORAN SELURUH ITEM TERJUAL', 0, 0, 'C');
$pdf->Ln();
...
$pdf->SetX(20);
$pdf->Cell(60, 8, 'JENIS ITEM', 1, 0, 'C', 1);
